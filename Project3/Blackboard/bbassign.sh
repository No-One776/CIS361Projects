...
# 3. Place all of the files that a student submitted into their subdir and removed the added prefix from each filename.
...
if [ $# -ne 1 ] # Checks if there is an file arguement
then
...
((k = 1))
while [ $(zipinfo -1 $1 | cut -c 1-$k | uniq | wc -l) -eq 1 ]
do
    ((k++))
done
((k--))
